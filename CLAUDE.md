...
13. **`BgCanvas` skips on mobile (`≤800px`).** Implemented via an outer `BgCanvas` shell component that returns `null` and an inner `BgCanvasInner` holding all hooks — required to avoid hooks-after-return violation. The `≤800px` check goes through the shared **`isPhoneViewport()` / `usePhoneViewport()`** (`src/config/breakpoints.ts` `PHONE_BREAKPOINT`, mirrored in SCSS as `$bp-phone` in `tokens.scss`) — never hardcode `innerWidth <= 800` or a bare `max-width: 800px` again. `breakpoints.ts`'s header holds the container-query-vs-viewport-`@media` ownership rule (ROADMAP §18).
